...
#training transcript data split
...
checkpoint_dir = "model_checkpoints_AUS"
loadedlength = 0
...
for epoch in range(loadedlength, epochs):
    ...
    if val_f1 >= max(val_f):
        ...
        if counter >= patience:
            ...
            break
# Save the best model
torch.save(best_model_state_dict, "best_model_AUS.pth")
...
metrics_df.to_csv('metricsAUS.csv', index=False)
...
# Save the DataFrame to a CSV file
df.to_csv('liberal_probabilities_by_year_by_justiceAUS.csv', index=False)
...
# Save the plot
plt.savefig('liberal_probability_by_year_by_justice_plotAUS.png')
# Show the plot
plt.show()
...
plt.savefig('liberal_probability_by_justice_plotAUS.png')
plt.show()
csv_filename = "aggregated_liberal_probabilitiesAUS.csv"
